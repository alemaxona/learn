...
# Прочитать про управления памятью через __slots__!
__slots__


# Правильное использование вывода:
# Неправильно
print('Max' + ' + ' + 'Python')

# Правильно! Экономит память и работает быстрее!
print('{} {} {}'.format('Max', '+', 'Python'))


# Декларативный код, можно назвать утверждающим, более простым, например:
class Config():
    a = 5
    b = 'str'
# Это декларативный код, так как он ничего не делает а только объявляет(классб переменные).
# Основное преимущество - легкая читаемость кода
# Если есть логика(условия, вычисления...), это уже недекларативный код!
