...
# Перегрузить(оператор) - Переопределить, например переменную \ поведение метода объекта.
...
class MathMethod(object):
    ...
    def __add__(self, other):  # Перегрузка сложения\ конкатенации
